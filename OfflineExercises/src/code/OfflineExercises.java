...
public String getSandwich(String input) throws StringIndexOutOfBoundsException  { 
		...
		int secondIndex = input.lastIndexOf("bread");
		String output = "";
		if(firstIndex != -1 && secondIndex != -1 && firstIndex != secondIndex) {
			output = input.substring(firstIndex + 5, secondIndex);
		}
		return output;
	}
...
public static boolean evenlySpaced(int a, int b, int c) {
	...
	public String nTwice(String input, int a) {
		String output = "";
		output = input.substring(0, a) + input.substring(input.length()-a, input.length());
		return output;
	}
